...
# Given
echo "STEP: Given — require pending help state with an active RPC connection"
...
echo "PREREQ: expected state includes workspace.pendingHelp.id=help-123, workspace.pendingHelp.rpc active or workspace.piRpc active, and workspace.id=ws-001"
cat > "$REQUEST_BODY_FILE" <<'EOF'
{"prompt":"The file is located in /data/reports folder"}
EOF

# When
echo "STEP: When — POST /api/v1/prompt to answer the agent help request"
...
# Then
echo "STEP: Then — assert prompt was accepted as a help response"
...
# Cleanup
echo "STEP: Cleanup — no public cleanup available for in-memory pendingHelp state"

echo "CODEVALID_TEST_ASSERTION_OK:pending_help_response_flow"
