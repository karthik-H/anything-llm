...
CASE_SUFFIX="$(date +%s)-$$"
CLIENT_SECRET="secret-only-${CASE_SUFFIX}"
TMP_DIR="$(mktemp -d)"
...
# Given
echo "STEP: Given — prepare request missing clientId"
...
{"clientSecret":"${CLIENT_SECRET}","authType":"common"}
EOF

echo "PREREQ: invalid request body prepared without clientId"
...
# When
echo "STEP: When — POST /admin/agent-skills/outlook/auth-url without clientId"
...
# Then
echo "STEP: Then — assert validation error for missing clientId"
...
echo "CODEVALID_TEST_ASSERTION_OK:validation_missing_client_id"
