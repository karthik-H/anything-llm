...
CASE_SUFFIX="$(date +%s)-$$"
CLIENT_ID="test-client-id-12345-${CASE_SUFFIX}"
CLIENT_SECRET="test-secret-abcde-${CASE_SUFFIX}"
TMP_DIR="$(mktemp -d)"
...
# Given
echo "STEP: Given — prepare valid common auth request"
...
echo "PREREQ: request body prepared for common auth type"
...
# When
echo "STEP: When — POST /admin/agent-skills/outlook/auth-url with valid common auth configuration"
...
# Then
echo "STEP: Then — assert successful auth URL generation for common authority"
...
grep -F 'redirect_uri' "$RESPONSE_BODY" >/dev/null 2>&1 || { echo "ASSERTION_FAILED: expected auth URL to include redirect_uri"; exit 1; }
...
echo "CODEVALID_TEST_ASSERTION_OK:happy_path_common_auth_type"
