...
# Given
echo "STEP: Given — prepare request body with skillName explicitly null"
echo "PREREQ: endpoint checks if (!skillName), so null should trigger required field validation"
cat > "$REQUEST_BODY_FILE" <<'EOF'
{"skillName":null}
EOF

# When
echo "STEP: When — POST /agent-skills/whitelist/add with null skillName"
...
# Then
echo "STEP: Then — assert validation rejects null skillName"
...
echo "CODEVALID_TEST_ASSERTION_OK:add_skill_skillname_null_value"
