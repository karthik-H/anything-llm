...
# Given
echo "STEP: Given — require retained subagent context with hypervisor still active"
...
echo "PREREQ: expected state includes workspace.subagentContext present, workspace.piRpc active, no active subagentRun, workspace.id=ws-001"
cat > "$REQUEST_BODY_FILE" <<'EOF'
{"prompt":"What were the results from the subagent task?"}
EOF

# When
echo "STEP: When — POST /api/v1/prompt after subagent context exists"
...
# Then
echo "STEP: Then — assert prompt was accepted in hypervisor mode"
...
# Cleanup
echo "STEP: Cleanup — no public cleanup available for in-memory subagent context"

echo "CODEVALID_TEST_ASSERTION_OK:continue_hypervisor_after_subagent_context"
