...
echo "PREREQ: this scenario requires server runtime state not exposed by any discovered public setup API"
echo "PREREQ: expected state includes configured OpenAI/base URL, workspace.id=ws-001, workspace.piRpc active, no pendingHelp, no active subagentRun"
cat > "$REQUEST_BODY_FILE" <<'EOF'
{"prompt":"Analyze the quarterly sales data and summarize trends"}
EOF

# When
echo "STEP: When — POST /api/v1/prompt with a valid prompt"
...
# Then
echo "STEP: Then — assert prompt was accepted and workspace id returned"
...
# Cleanup
echo "STEP: Cleanup — no public cleanup available for in-memory workspace state"

echo "CODEVALID_TEST_ASSERTION_OK:send_prompt_happy_path"
