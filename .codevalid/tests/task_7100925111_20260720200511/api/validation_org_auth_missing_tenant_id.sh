...
CASE_SUFFIX="$(date +%s)-$$"
CLIENT_ID="org-client-999-${CASE_SUFFIX}"
CLIENT_SECRET="org-secret-999-${CASE_SUFFIX}"
TMP_DIR="$(mktemp -d)"
...
# Given
echo "STEP: Given — prepare organization auth request missing tenantId"
...
{"clientId":"${CLIENT_ID}","clientSecret":"${CLIENT_SECRET}","authType":"organization"}
EOF

echo "PREREQ: invalid organization request body prepared without tenantId"
...
# When
echo "STEP: When — POST /admin/agent-skills/outlook/auth-url with organization authType but no tenantId"
...
# Then
echo "STEP: Then — assert validation error for missing tenantId"
...
grep -F 'Tenant ID is required for organization-only authentication.' "$RESPONSE_BODY" >/dev/null 2>&1 || { echo "ASSERTION_FAILED: expected missing tenantId validation error"; exit 1; }

# Cleanup
echo "STEP: Cleanup — no cleanup required for rejected validation request"

echo "CODEVALID_TEST_ASSERTION_OK:validation_org_auth_missing_tenant_id"
