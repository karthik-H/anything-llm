...
# Given
echo "STEP: Given — verify authenticated availability-check precondition assumptions"
echo "PREREQ: this scenario requires a valid authenticated session accepted by validatedRequest middleware"
echo "PREREQ: this scenario also requires the create-files-agent plugin to be installed and isToolAvailable() to return true in the running environment"
...
# Then
echo "STEP: Then — assert HTTP 200 and available true response"
...
echo "CODEVALID_TEST_ASSERTION_OK:create_files_agent_available_returns_true"
