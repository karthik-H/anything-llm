...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="add_skill_anonymous_single_user_mode"
...
echo "STEP: Given — anonymous single-user mode context is required"
echo "PREREQ: this test must run in an environment where no valid user session exists and multi-user mode is disabled or unset"

echo "STEP: When — POST /agent-skills/whitelist/add anonymously in single-user mode"
...
echo "STEP: Then — anonymous add succeeds"
...
echo "CODEVALID_TEST_ASSERTION_OK:add_skill_anonymous_single_user_mode"
