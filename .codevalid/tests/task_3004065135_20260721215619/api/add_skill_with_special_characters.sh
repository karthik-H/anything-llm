...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="add_skill_with_special_characters"
SKILL_NAME="web_scraping-v2-${CASE_SUFFIX}"
...
echo "STEP: Given — authenticated caller context assumed and special-character skill prepared"
echo "PREREQ: this test assumes the environment provides a valid authenticated session accepted by the app"

echo "STEP: When — POST /agent-skills/whitelist/add with a special-character skill name"
...
echo "STEP: Then — special-character skill add succeeds"
...
echo "CODEVALID_TEST_ASSERTION_OK:add_skill_with_special_characters"
