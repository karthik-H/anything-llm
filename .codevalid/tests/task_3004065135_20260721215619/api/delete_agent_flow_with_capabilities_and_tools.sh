...
CASE_SUFFIX="$(date +%s)-$$"
FLOW_UUID="flow-cap-${CASE_SUFFIX}"
AUTH_HEADER="${AUTH_HEADER:-Authorization: Bearer admin-token-${CASE_SUFFIX}}"

WHEN_RESPONSE_HEADERS="/tmp/delete_agent_flow_with_capabilities_and_tools_when_headers_${CASE_SUFFIX}.txt"
WHEN_RESPONSE_BODY="/tmp/delete_agent_flow_with_capabilities_and_tools_when_body_${CASE_SUFFIX}.txt"
CLEANUP_RESPONSE_HEADERS="/tmp/delete_agent_flow_with_capabilities_and_tools_cleanup_headers_${CASE_SUFFIX}.txt"
CLEANUP_RESPONSE_BODY="/tmp/delete_agent_flow_with_capabilities_and_tools_cleanup_body_${CASE_SUFFIX}.txt"
...
# Given
echo "STEP: Given — prepare admin delete request for flow with capabilities and tools"
echo "PREREQ: using admin authorization header and unique flow uuid ${FLOW_UUID}"

# When
echo "STEP: When — send DELETE request for agent flow with capabilities and tools"
...
# Then
echo "STEP: Then — assert delete succeeded for flow with capabilities and tools"
...
# Cleanup
echo "STEP: Cleanup — best-effort idempotent delete retry for capabilities-and-tools test uuid"
echo 'REQUEST_HEADERS: Content-Type: application/json'
echo "REQUEST_HEADERS: ${AUTH_HEADER}"
echo 'REQUEST_BODY: '
...
echo 'CODEVALID_TEST_ASSERTION_OK:delete_agent_flow_with_capabilities_and_tools'
