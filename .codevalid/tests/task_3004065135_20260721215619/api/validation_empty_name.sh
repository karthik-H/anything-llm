...
# Given
echo "STEP: Given — prepare request with empty name string"
echo "PREREQ: building a payload where name is present but empty"

# When
echo "STEP: When — submit save flow request with empty name"
...
# Then
echo "STEP: Then — verify empty name is rejected"
...
grep -F 'Name and config are required' "$RESP_BODY_FILE" >/dev/null || { echo "ASSERTION_FAILED: expected validation error message for empty name"; exit 1; }

# Cleanup
echo "STEP: Cleanup — no cleanup required for stateless validation request"

echo "CODEVALID_TEST_ASSERTION_OK:validation_empty_name"
