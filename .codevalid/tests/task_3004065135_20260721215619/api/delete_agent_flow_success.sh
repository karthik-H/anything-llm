...
CASE_SUFFIX="$(date +%s)-$$"
FLOW_UUID="flow-success-${CASE_SUFFIX}"
AUTH_HEADER="${AUTH_HEADER:-Authorization: Bearer admin-token-${CASE_SUFFIX}}"

WHEN_RESPONSE_HEADERS="/tmp/delete_agent_flow_success_when_headers_${CASE_SUFFIX}.txt"
WHEN_RESPONSE_BODY="/tmp/delete_agent_flow_success_when_body_${CASE_SUFFIX}.txt"
CLEANUP_RESPONSE_HEADERS="/tmp/delete_agent_flow_success_cleanup_headers_${CASE_SUFFIX}.txt"
CLEANUP_RESPONSE_BODY="/tmp/delete_agent_flow_success_cleanup_body_${CASE_SUFFIX}.txt"
...
# Given
echo "STEP: Given — prepare unique admin delete request context"
echo "PREREQ: using admin authorization header and unique flow uuid ${FLOW_UUID}"

# When
echo "STEP: When — send DELETE request for agent flow as admin"
...
# Then
echo "STEP: Then — assert delete succeeded or surface contract mismatch"
...
# Cleanup
echo "STEP: Cleanup — best-effort idempotent delete of test flow uuid"
echo 'REQUEST_HEADERS: Content-Type: application/json'
echo "REQUEST_HEADERS: ${AUTH_HEADER}"
echo 'REQUEST_BODY: '
...
echo 'CODEVALID_TEST_ASSERTION_OK:delete_agent_flow_success'
