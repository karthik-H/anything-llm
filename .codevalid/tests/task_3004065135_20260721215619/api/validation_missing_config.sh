...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="validation_missing_config"
...
{
  "name": "Test Agent ${CASE_SUFFIX}"
}
EOF

# Given
echo "STEP: Given — prepare request missing required config"
echo "PREREQ: building a payload that omits the config field"

# When
echo "STEP: When — submit save flow request without config"
...
# Then
echo "STEP: Then — verify validation rejects missing config"
...
grep -F 'Name and config are required' "$RESP_BODY_FILE" >/dev/null || { echo "ASSERTION_FAILED: expected validation error message for missing config"; exit 1; }

# Cleanup
echo "STEP: Cleanup — no cleanup required for stateless validation request"

echo "CODEVALID_TEST_ASSERTION_OK:validation_missing_config"
